//  gcc -c gauss.c -O3 -o gauss.o
//  ᓚᘏᗢ
/*  --------------------------------------------------------------------------------------------------------
*
*   The MIT License terms apply https://opensource.org/licenses/MIT.
*
*   This algorithm has been adapted to C from the C# Redzen general purpose library:
...
*   --------------------------------------------------------------------------------------------------------
    ...
    approach for the algorithm as described above and implemented here. To explain
...
// x | Area = x*exp(-x*x/2)+sqrt(pi/2)*erfc(x/sqrt(2)), x = R0
#define Right 3.442619855899

// Area of each rectangle (pre-determined/computed for 128 blocks).
// Area = R0*Y0 + Tail, Y0 = exp(-R0*R0/2), Tail = sqrt(pi/2)*erfc(R0/sqrt(2))
#define Area 9.91256303526217e-3

// Denominator for INCR constant. This is the number of distinct values this algorithm is capable
// of generating in the interval [0,1], i.e. (2^53)-1 distinct values.
#define MAXINT ((1UL << 53) - 1)

// Scale factor for converting a ULong with interval [0, 0x1f_ffff_ffff_ffff] to a double with interval [0,1].
#define INCR (1.0 / (double) MAXINT)

// x[i] and y[i] describe the top-right position of rectangle i.
// Allocate storage. We add one to the length of _x so that we have an entry at x[blockCount], this avoids having
// to do a special case test when sampling from the top box.
// Area_Div_Y0 is the area A divided by the height of B0. Note. This is *not* the same as x[i] because the area
// of B0 is A minus the area of the distribution tail.
// num is the number sampled from the gaussian.
double x[blockCount+1], y[blockCount], Area_Div_Y0, num;
...
uint64_t xComp[blockCount];

// Denormalised gaussian probability density function: y = e^-(x^2/2).
double GaussianPdfDenorm (double _x) {
    return exp(-(_x*_x*.5));
}

// Inverse function of GaussianPdfDenorm(x).
double GaussianPdfDenormInv (double _y) {
    ...
    return sqrt(-2.*log(_y));
}

static inline double DoubleFromBits(uint64_t i) {
	return (i >> 11) * 0x1.0p-53;
}

/// Sample from the distribution tail (defined as having x >= Right).
void SampleTail (double *_x) {
    double _y;
    do {
        // Note. The program doesn't check if next() returns 0.; however, the possibility
        // it does are 2^-63 (approx 10^-19) which is unlikely for the intended purpose of
        // this program.
        *_x = -log(DoubleFromBits(XoshiroNext())) / Right;
        _y = -log(DoubleFromBits(XoshiroNext()));
    } while (_y+_y < (*_x)*(*_x));
    (*_x) += Right;
    return;
}

static inline void SetSignBit (double *_x, uint64_t *signBit) {
    (*(uint64_t *) _x) |= (*signBit);
    return;
}

// Gets a random value sampled from the standard Gaussian distribution: N(0,1).
double SampleStd () {
    for (;;) {
        // Generate 64 random bits.
        uint64_t u = XoshiroNext();
        ...
        if (s == 0) {
            if(u2 < xComp[0]) // Generated x is within R0.
                num = u2 *  INCR * Area_Div_Y0;
            else // Generated x is in the tail of the distribution.
                SampleTail(&num);
            
            SetSignBit(&num, &signBit);
            return num;
        }

        // All other segments.
        if (u2 < xComp[s]) { // Generated x is within the rectangle.
            num = u2 * (INCR) * x[s];
            SetSignBit(&num, &signBit);
            return num;
        }
        ...
        // This execution path is relatively slow/expensive (makes a call to exp()) but is relatively rarely executed,
        // although more often than the 'tail' path (above).
        num = u2 * INCR * x[s];
        if (y[s-1] + ((y[s] - y[s-1]) * DoubleFromBits(XoshiroNext())) < GaussianPdfDenorm(num)) {
            SetSignBit(&num, &signBit);
            return num;
        }
    }
...
// Gets a random value sampled from the Gaussian distribution with arbitrary mean and standard deviation.
double Sample (double mean, double stdDev) {
    return (SampleStd() * stdDev) + mean;
}

void SetupGaussZiggurat() {
    // Call to setup the xoshiro256+ PRNG algorithm for 64-bit doubles uniformly distributed
    // in [0,1].
    SetupXoshiro();
    ...
    // Calculate the positions of all remaining rectangles.
    ...
    xComp[0] = (uint64_t) (((Right * y[0]) / Area) * (double) MAXINT);

    for (int i = 1; i < blockCount - 1; ++i)
        xComp[i] = (uint64_t)((x[i+1] / x[i]) * (double) MAXINT);
